...
def run(sample, save_dir):
    ...
    data_path = f"vis_results/tartanair/tape3d_pairwise_stride2_depthcrafter/{vid_name}_pred_dict_pairwise.pkl"
    ...
    B, T = video.shape[:2]
    video = F.interpolate(video.flatten(0,1), size=(384, 512), mode='bilinear', align_corners=False).reshape(B,T,3,384,512)
    videodepth = F.interpolate(videodepth.flatten(0,1), size=(384, 512), mode='nearest').reshape(B,T,1,384,512)
    if videomask is not None:
        videomask = F.interpolate(videomask.float().flatten(0,1), size=(384, 512), mode='nearest').reshape(B,T,1,384,512).bool()
    ...
    #     for j in range(i):
    #         k_i, k_j = keyframe_id_list[i], keyframe_id_list[j]

    #         # NOTE remove far-away pairs
    #         if abs(k_i - k_j) > chunk_size:
    #             continue
    ...
    for pair, pair_id in zip(pairs, pairs_id):
        prepare_single_pair(pair, pair_id, data_dict, keyframe_id_list, keyframe_masks, view1, view2, pred1, pred2)

        # pair_swap = (pair[1], pair[0])
        # pair_id_swap = (pair_id[1], pair_id[0])

        # prepare_single_pair(pair_swap, pair_id_swap, data_dict, keyframe_id_list, keyframe_masks, view1, view2, pred1, pred2)

    if len(view1['img']) <= num_keyframes // 2:
    ...
    pred1['flow3d'] = torch.stack(pred1['flow3d']).cuda()
    ...
    # for keyframe_id in data_dict.keys():
    #     # keyframe_id_list.append(keyframe_id)

    #     d_dict = data_dict[keyframe_id]
    device ="cuda"
    num_total_iter = 200
    schedule = "linear"
    lr = 0.01
    # optim_kw = {'verbose': True, 'min_conf_thr': 0.5, 'save_path': save_path, 'disable_flow_loss': True}

    log_dir = f'logdirs/dust3r_pose_consist_depth_tum/{vid_name}'

    optim_kw = {'verbose': True, 'min_conf_thr': 0.5, 'log_dir': log_dir, 'num_total_iter': num_total_iter}
    scene = PointCloudOptimizer(view1, view2, pred1, pred2, **optim_kw).to(device)

    
        # scene._set_depthmap(0, last_depthmap)
        # scene.freeze_depthmap(0)

    

    loss = scene.compute_global_alignment(init='mst', niter=num_total_iter, schedule=schedule, lr=lr)
    ...
    # last_depthmap = scene.get_depthmaps()[0].detach()

    # breakpoint()
    # [p[m] for p, m in zip(pts3d, msk)]
    # col = [p[m] for p, m in zip(rgbimg, msk)]
    ...
    with open(os.path.join(save_path, f'{vid_name}.pkl'), 'wb') as handle:
        pickle.dump(save_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)

    gt_traj = load_traj(
        f"TartanAir/TartanAir_shibuya/{vid_name}/gt_pose.txt",
        "tartanair",
        skip=0,
        stride=1,
    )
    ...
    try:
        ate, rpe_trans, rpe_rot = eval_metrics(
            ...
            gt_traj=gt_traj,
            seq=vid_name,
            filename=os.path.join(save_dir, f"eval_metrics_{vid_name}.txt"),
        )
    except:
        ate, rpe_trans, rpe_rot = np.nan, np.nan, np.nan
...
if __name__ == "__main__":
    ...
    save_dir = "results/eval_traj_tartanair_depthcrafter"
    os.makedirs(save_dir, exist_ok=True)

    avg_ate, avg_rpe_trans, avg_rpe_rot = [], [], []
    for ind, sample in tqdm(enumerate(test_dataloader)):

        if os.path.exists(os.path.join(save_dir, f"eval_metrics_{sample.seq_name[0]}.txt")):
            print(f"skip {sample.seq_name[0]}")
            continue


        # if sample.seq_name[0] in ['market_6']:
        #     continue

        ate, rpe_trans, rpe_rot = run(sample, save_dir)
        # break
    ...
    eval_files = [f for f in os.listdir(save_dir) if f.endswith(".txt")]

    new_avg_ate, new_avg_rpe_trans, new_avg_rpe_rot = [], [], []
    for eval_file in eval_files:
        with open(os.path.join(save_dir, eval_file), 'r') as f:
            lines = f.readlines()
