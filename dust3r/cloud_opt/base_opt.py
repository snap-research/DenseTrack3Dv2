...
import dust3r.cloud_opt.delta_init_im_poses as delta_init_fun

from densetrack3d.utils.logger import Logger
from densetrack3d.evaluation.core.traj_metrics import save_trajectory_tum_format
...
class BasePCOptimizer (nn.Module):
    ...
    def _init_from_views(self, view1, view2, pred1, pred2,
        ...
        # if 'flow3d' in pred1.keys():
        #     self.flow3d_i = NoGradParamDict({ij: pred1['flow3d'][n] for n, ij in enumerate(self.str_edges)})
        if 'flow2d' in pred1.keys():
            self.flow2d_i = NoGradParamDict({ij: pred1['flow2d'][n] for n, ij in enumerate(self.str_edges)})
        if 'visib' in pred1.keys():
            self.visib_i = NoGradParamDict({ij: pred1['visib'][n] for n, ij in enumerate(self.str_edges)})

        
        # if 'flow3d' in pred2.keys():
        #     self.flow3d_j = NoGradParamDict({ij: pred2['flow3d'][n] for n, ij in enumerate(self.str_edges)})
        if 'flow2d' in pred2.keys():
            self.flow2d_j = NoGradParamDict({ij: pred2['flow2d'][n] for n, ij in enumerate(self.str_edges)})
        if 'visib' in pred2.keys():
            self.visib_j = NoGradParamDict({ij: pred2['visib'][n] for n, ij in enumerate(self.str_edges)})
        # else:
        #     self.flow3d_i = None
        ...
        self.input_dynamic_masks = None

        dymask_i = {ij: torch.zeros_like(pred1_pts[n][...,0]) for n, ij in enumerate(self.str_edges)}
        dymask_j = {ij: torch.zeros_like(pred2_pts[n][...,0]) for n, ij in enumerate(self.str_edges)}
        if 'dynamic_mask' in view1 and 'dynamic_mask' in view2 and view1['dynamic_mask'] is not None and view2['dynamic_mask'] is not None:
            self.input_dynamic_masks = [torch.zeros((1,)+hw) for hw in self.imshapes]
            for v in range(len(self.edges)):
                idx = view1['idx'][v]
                self.input_dynamic_masks[idx] = view1['dynamic_mask'][v]
                idx = view2['idx'][v]
                self.input_dynamic_masks[idx] = view2['dynamic_mask'][v]

                dymask_i[self.str_edges[v]] = view1['dynamic_mask'][v]
                dymask_j[self.str_edges[v]] = view2['dynamic_mask'][v]

        self.dymask_i = NoGradParamDict(dymask_i)
        self.dymask_j = NoGradParamDict(dymask_j)
            # self.dynamic_masks = torch.stack(dynamic_masks)
        # breakpoint()
    ...
    def compute_global_alignment(self, init=None, niter_PnP=10, stage='1', use_delta_init=False, **kw):
        if init is None:
            pass
        elif init == 'msp' or init == 'mst':
            if use_delta_init:
                delta_init_fun.init_minimum_spanning_tree(self, niter_PnP=niter_PnP)
            else:
        ...
        return global_alignment_loop(self, logger=self.logger, stage=stage, **kw)
...
def global_alignment_loop(net, lr=0.01, niter=300, schedule='cosine', lr_min=1e-6, logger=None, stage='1'):
    params = [p for p in net.parameters() if p.requires_grad]
    if not params:
        return net
    
    total_params = sum(p.numel() for p in params)
    print("Total learnable params: ", total_params)
    ...
    if verbose:
        with tqdm.tqdm(total=niter) as bar:
            while bar.n < bar.total:
                loss, lr = global_alignment_iter(net, bar.n, niter, lr_base, lr_min, optimizer, schedule, logger=logger, stage=stage)
                bar.set_postfix_str(f'{lr=:g} loss={loss:g}')
                bar.update()
    else:
        for n in range(niter):
            loss, _ = global_alignment_iter(net, n, niter, lr_base, lr_min, optimizer, schedule, logger=logger, stage=stage)
    return loss



def global_alignment_iter(net, cur_iter, niter, lr_base, lr_min, optimizer, schedule, logger=None, stage='1'):
    t = cur_iter / niter
    if schedule == 'cosine':
    ...
    # align_loss_lambda = linear_schedule(t, 100.0, 0.1)
    # # flow_loss_lambda = linear_schedule(1 - t, 10.0, 0.1)
    # net.loss_weight_dict['align_loss'] = align_loss_lambda
    # net.loss_weight_dict['flow_loss'] = flow_loss_lambda

    
    # FIXME update loss weight
    # if cur_iter == int(niter * 0.7):
    #     old_weight = net.loss_weight_dict['flow_loss']
    #     net.loss_weight_dict['flow_loss'] = old_weight * 10
    #     print(f"Adjust flow loss weight from {old_weight} -> {net.loss_weight_dict['flow_loss']}")

    #     old_weight = net.loss_weight_dict['align_loss']
    #     net.loss_weight_dict['align_loss'] = old_weight * 0.01
    #     print(f"Adjust align loss weight from {old_weight} -> {net.loss_weight_dict['align_loss']}")
    
    if logger is not None:
        logger.writer.add_scalar("learning_rate", lr, cur_iter)
        for k, v in net.loss_weight_dict.items():
            logger.writer.add_scalar(f"{k}_weight", v, cur_iter)

    loss_dict = net(cur_iter, stage=stage)
    ...
    # loss_dict['']
        if logger is not None:
            logger.writer.add_scalar(f"{k}", v.item(), cur_iter)
