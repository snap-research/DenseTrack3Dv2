...
class TapVid2DDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if self.dataset_type == "kinetics":
            ...
            for subset_id in range(10):
                ...
                data_path = os.path.join(data_root, f"tapvid_{dataset_type}", f"{subset_id:04d}_of_0010.pkl")
                data = (
    ...
    def __getitem__(self, index):
        if self.num_processes >= 0 and self.num_processes > len(self.points_dataset):
            index = index % len(self.points_dataset)
        ...
        if "depth_preds" in sample.keys() and sample["depth_preds"] is not None:
            ...
            if self.read_from_s3:
                depths = np.load(get_client_stream(self.client, depth_path))["depth"][..., None]
            else:
        ...
        # depths = depths[:max_t]
        # segs = segs[:max_t]
        # mask_valid = query_points[:, 0] < (max_t - 1)
        # query_points = query_points[mask_valid]
        # trajs = trajs[:max_t, mask_valid]
        # visibles = visibles[:max_t, mask_valid]
