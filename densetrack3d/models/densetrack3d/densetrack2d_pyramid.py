...
class DenseTrack2DPyramid(nn.Module):
    def __init__(
        self,
        window_len=8,
        stride=4,
        ...
        self.stride = stride
        self.pyramid_stride = pyramid_stride
    ...
    def forward_window(
        ...
        for iteration in range(iters):
            ...
            fcorrs = self.get_4dcorr_features(
                fmaps_pyramid[pyr_start:pyr_end], 
                coords, 
                supp_track_feat[iteration]
            )
            ...
            coords_sparse = coords[:, :, :self.N_sparse] * 2
            coords_dense = coords[:, :, self.N_sparse:] * 2
            coords_dense = rearrange(coords_dense, 'b t (h w) c -> (b t) c h w', h=self.dH, w=self.dW)
            coords_dense = F.interpolate(coords_dense, scale_factor=2, mode='bilinear', align_corners=False)
            coords_dense = rearrange(coords_dense, '(b t) c h w -> b t (h w) c', b=B, t=S)

            track_feat_sparse = track_feat[:, :, :self.N_sparse]
            track_feat_dense = track_feat[:, :, self.N_sparse:]
            track_feat_dense = rearrange(track_feat_dense, 'b t (h w) c -> (b t) c h w', h=self.dH, w=self.dW)
            track_feat_dense = F.interpolate(track_feat_dense, scale_factor=2, mode='bilinear', align_corners=False)
            track_feat_dense = rearrange(track_feat_dense, '(b t) c h w -> b t (h w) c', b=B, t=S)

            coords = torch.cat([coords_sparse, coords_dense], dim=2)
            track_feat = torch.cat([track_feat_sparse, track_feat_dense], dim=2)
            ########################################################################
    ...
    def extract_features(self, video: VideoType) -> tuple[Float[Tensor, "*"], ...]:
        ...
        pyr_fmaps = self.fnet(
            rearrange(video, "b t c h w -> (b t) c h w"), pyramid=True
        )

        pyr_fmaps = [rearrange(f, "(b t) c h w -> b t c h w", b=B, t=T) for f in pyr_fmaps]
    ...
    def prepare_dense_queries(
        ...
        pyr_dense_track_feat, pyr_dense_supp_track_feats_pyramid = [], []
        for lvl, pyr in enumerate(self.pyramid_levels):
            # dH = dH // pyr
            # dW = dW // pyr
            pyr_start = lvl
            pyr_end = lvl + 3
            ...
            dense_grid_2d = (
                get_grid(dH // pyr, dW // pyr, normalize=False, device=fmaps.device).reshape(-1, 2).unsqueeze(0).repeat(B, 1, 1)
            )  # B, (H, W) 2
            for b in range(B):
                dense_grid_2d[b, :, 0] += (x0[b] // pyr)
                dense_grid_2d[b, :, 1] += (y0[b] // pyr)

            dense_track_feat, dense_supp_track_feats_pyramid = self.get_dense_track_feat(
                fmaps_pyramid[pyr_start+1],
                fmaps_pyramid[pyr_start:pyr_end],
                dense_grid_2d,
            )
            ...
            pyr_dense_track_feat.append(dense_track_feat)
            pyr_dense_supp_track_feats_pyramid.append(dense_supp_track_feats_pyramid)
        ...
        return dense_coords_init, dense_vis_init, dense_conf_init, pyr_dense_track_feat, pyr_dense_supp_track_feats_pyramid, (x0, y0)
    ...
    def forward(
        ...
        fmaps_pyramid = self.extract_features(video)
        # fmaps_pyramid = [higher_fmaps, fmaps, lower_fmaps]
        fH, fW = fmaps_pyramid[1].shape[-2:]
        ...
        if use_sparse:
            (
                ...
                supp_track_feats_pyramid,
                sparse_queried_frames,
            ) = self.prepare_sparse_queries(sparse_queries, fmaps_pyramid[1], fmaps_pyramid)
