...
from densetrack3d.models.model_utils import bilinear_sample2d, get_points_on_a_grid, smart_cat, get_grid
...
class Predictor2D(torch.nn.Module):
    ...
    def _compute_sparse_tracks(
        ...
        if queries is not None:
            ...
            grid_pts = (
                get_grid(self.interp_shape[0], self.interp_shape[1], normalize=False, device=video.device)
            )  # B, (H, W) 2
            grid_pts = grid_pts.reshape(1, self.interp_shape[0], self.interp_shape[1], 2)[:, ::16, ::16]
            grid_pts = grid_pts.reshape(1, -1, 2)  # B, (H, W) 2
            # grid_pts = grid_pts + 0.1
