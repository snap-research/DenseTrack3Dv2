...
class Asset(tl.HasTraits):
    ...
    def keyframe_insert(self, member: str, frame: int):
        ...
        self.notify_change(munch.Munch(name=member, owner=self, frame=frame, type="keyframe"))
    ...
    def at_frame(self, frame, interpolation="linear"):
        ...
        try:
            for key, value in self.keyframes.items():
                ...
                setattr(self, key, self.get_value_at(name=key, frame=frame, interpolation=interpolation))
            yield self
        finally:
    ...
    def get_values_over_time(self, name, frames=None, interpolation="linear"):
        if frames is None:
            frames = list(range(self.active_scene.frame_start, self.active_scene.frame_end + 1))
        return np.array(
            [self.get_value_at(name, frame=f, interpolation=interpolation) for f in frames], dtype=np.float32
        )
