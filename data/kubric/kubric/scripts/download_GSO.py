...
from kubric import file_io, redirect_io
...
def get_metadata(a):
    ...
    return {
        ...
        "url": "https://fuel.ignitionrobotics.org/1.0/GoogleResearch/models/{name}/1/{name}.zip".format(
...
def download_all(assets_list, num_processes: int = 16, download_dir: PathLike = "GSO_raw"):
...
def main(download_dir: PathLike = "GSO_raw", target_dir: PathLike = "GSO", keep_raw_assets=False):
    ...
    with tqdm.tqdm(total=len(assets)) as pbar:
        with multiprocessing.Pool(32, maxtasksperchild=1) as pool:
            promise = pool.imap_unordered(
                functools.partial(kubricify, source_dir=download_dir, target_dir=target_dir), assets
            )
    ...
    manifest = {"name": "GSO", "data_dir": str(target_dir), "version": "1.0", "assets": {k: v for k, v in assets_list}}
...
if __name__ == "__main__":
    ...
    main(download_dir=FLAGS.download_dir, target_dir=FLAGS.target_dir, keep_raw_assets=FLAGS.keep_raw_assets)
