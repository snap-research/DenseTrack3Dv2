...
def collect_list_of_available_assets(catalogue_path="hdri_haven_catalogue.json"):
...
def main(download_dir: PathLike = "GSO_raw", target_dir: PathLike = "GSO", keep_raw_assets=False):
    ...
    with tqdm.tqdm(total=len(catalogue)) as pbar:
        with multiprocessing.Pool(32, maxtasksperchild=1) as pool:
            promise = pool.imap_unordered(
                functools.partial(kubricify, source_dir=download_dir, target_dir=target_dir), catalogue
            )
    ...
    manifest = {"name": "HDRI_haven", "data_dir": str(target_dir), "version": "1.0", "assets": assets}
