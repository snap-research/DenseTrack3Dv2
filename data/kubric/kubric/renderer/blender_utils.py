...
def add_coordinate_material():
    ...
    mat.node_tree.links.new(tex_coordinates.outputs.get("Generated"), aov_out_node.inputs.get("Color"))
    mat.node_tree.links.new(tex_coordinates.outputs.get("Generated"), unused_mat_out_node.inputs.get("Surface"))

    return mat


def activate_render_passes(
    normal: bool = True, optical_flow: bool = True, segmentation: bool = True, uv: bool = True, depth: bool = True
):
...
def get_render_layers_from_exr(filename) -> Dict[str, np.ndarray]:
    ...
    if "Image" in layer_names:
        ...
        output["linear_rgba"] = read_channels_from_exr(exr, ["Image.R", "Image.G", "Image.B", "Image.A"])
...
def replace_cryptomatte_hashes_by_asset_index(segmentation_ids: ArrayLike, assets: Sequence[core.assets.Asset]):
...
def apply_transformations(
    objects: Union[bpy.types.Object, Sequence[bpy.types.Object]], position=False, rotation=True, scale=True
):
...
def process_segementation(exr_layers, scene):  # pylint: disable=unused-argument
    ...
    return replace_cryptomatte_hashes_by_asset_index(exr_layers["segmentation_indices"][:, :, :1], scene.assets)
