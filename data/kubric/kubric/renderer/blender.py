...
import tqdm
from kubric import core, file_io
...
class Blender(core.View):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            scene_observers={
                ...
                "resolution": [
                    AttributeSetter(self.blender_scene.render, "resolution_x", converter=lambda x: x[0]),
                    AttributeSetter(self.blender_scene.render, "resolution_y", converter=lambda x: x[1]),
                ],
                "camera": [AttributeSetter(self.blender_scene, "camera", converter=self._convert_to_blender_object)],
    ...
    def use_gpu(self, value: bool):
        ...
        if value:
            ...
            devices_used = [d.name for d in bpy.context.preferences.addons["cycles"].preferences.devices if d.use]
    ...
    def render(
        ...
        for frame_nr in tqdm.tqdm(frames):
            with RedirectStream(stream=sys.stdout, disabled=self.verbose):
                ...
                bpy.context.scene.render.filepath = str(self.scratch_dir / "images" / f"frame_{frame_nr:04d}.png")
                bpy.ops.render.render(animation=False, write_still=True)
                logger.info("Rendered frame '%s'", bpy.context.scene.render.filepath)

        # BVH MOD / NOTE: Otherwise we sometimes get:
        # Error reading pixel data from image file "/tmp/mygenkub_kubmv_d3/988471540/exr/frame_0017.exr". Early end of file: read 0 out of 4 requested bytes.
        time.sleep(0.5)
        # NOTE ^ the issue was probably due to tmp folder collision instead
    ...
    def _check_missing_textures(self):
        missing_textures = sorted(
            {img.filepath for img in bpy.data.images if tuple(img.size) == (0, 0) and img.filepath}
        )
        if missing_textures:
    ...
    def render_still(
        ...
        result = self.render(
            frames=[frame], ignore_missing_textures=ignore_missing_textures, return_layers=return_layers
        )
    ...
    def postprocess(self, from_dir: PathLike, return_layers: Sequence[str]):
        ...
        png_frames = [from_dir / "images" / (exr_filename.stem + ".png") for exr_filename in exr_frames]
    ...
    def _add_asset(self, asset: core.Cube):
        ...
        asset.observe(AttributeSetter(cube, "active_material", converter=self._convert_to_blender_object), "material")
    ...
    def _add_asset(self, obj: core.Sphere):
        ...
        obj.observe(AttributeSetter(sphere, "active_material", converter=self._convert_to_blender_object), "material")
    ...
    def _add_asset(self, obj: core.FileBasedObject):
                ...
                with redirect_stdout(fstdout):  # < also suppresses python stdout
                    if extension == "obj":
                        bpy.ops.import_scene.obj(
                            filepath=obj.render_filename, use_split_objects=False, **obj.render_import_kwargs
                        )
                    elif extension in ["glb", "gltf"]:
                        bpy.ops.import_scene.gltf(filepath=obj.render_filename, **obj.render_import_kwargs)
                        ...
                        bpy.ops.import_scene.fbx(filepath=obj.render_filename, **obj.render_import_kwargs)
                    elif extension in ["x3d", "wrl"]:
                        bpy.ops.import_scene.x3d(filepath=obj.render_filename, **obj.render_import_kwargs)
        ...
        obj.observe(
            AttributeSetter(blender_obj, "active_material", converter=self._convert_to_blender_object), "material"
        )
    ...
    def _add_asset(self, obj: core.PrincipledBSDFMaterial):
        ...
        obj.observe(
            KeyframeSetter(bsdf_node.inputs["Specular Tint"], "default_value"), "specular_tint", type="keyframe"
        )
        ...
        obj.observe(
            AttributeSetter(bsdf_node.inputs["Transmission Roughness"], "default_value"), "transmission_roughness"
        )
        obj.observe(
            KeyframeSetter(bsdf_node.inputs["Transmission Roughness"], "default_value"),
    ...
    def _add_asset(self, obj: core.FlatMaterial):
        ...
        obj.observe(KeyframeSetter(holdout_mix_node.inputs["Fac"], "default_value"), "holdout", type="keyframe")
        obj.observe(AttributeSetter(indirect_mix_node.inputs["Fac"], "default_value"), "indirect_visibility")
        obj.observe(
            KeyframeSetter(indirect_mix_node.inputs["Fac"], "default_value"), "indirect_visibility", type="keyframe"
        )
        return mat
    ...
    def _set_ambient_light_hdri(self, hdri_filepath=None, hdri_rotation=(0.0, 0.0, 0.0), strength=1.0):
        # ensure hdri_node is connected
        self.blender_scene.world.node_tree.links.new(
            self.ambient_hdri_node.outputs.get("Color"), self.ambient_node.inputs.get("Color")
        )
