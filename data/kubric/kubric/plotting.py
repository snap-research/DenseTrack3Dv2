...
def hls_palette(n_colors, first_hue=0.01, lightness=0.5, saturation=0.7):
    """Get a list of colors where the first is black and the rest are evenly spaced in HSL space."""
    hues = np.linspace(0, 1, int(n_colors) + 1)[:-1]
    hues = (hues + first_hue) % 1
    palette = [(0.0, 0.0, 0.0)] + [colorsys.hls_to_rgb(h_i, lightness, saturation) for h_i in hues]
...
def get_image_plot(width, height, nrows=1, ncols=1, display_dpi=1):
    ...
    fig, axes = plt.subplots(
        figsize=(width * ncols, height * nrows), nrows=nrows, ncols=ncols, sharex=True, sharey=True, dpi=display_dpi
    )
    if nrows == ncols == 1:
        ax_list = [axes]
    else:
...
def plot_depth(depth, ax=None, depth_range=None, colormap="inferno_r"):
    ...
    ax.imshow((depth - depth_range[0]) / (depth_range[1] - depth_range[0]), cmap=colormap, interpolation="nearest")
...
def plot_segmentation(seg, ax=None, palette=None, num_objects=None):
    ...
    if palette is None:
        palette = [(0.0, 0.0, 0.0)] + sns.color_palette("hls", num_objects - 1)
...
def plot_flow(vec, ax=None, flow_mag_range=None):
    ...
    magnitude = np.clip((norm - flow_mag_range[0]) / (flow_mag_range[1] - flow_mag_range[0]), 0.0, 1.0)
...
def plot_bboxes(seg, ax=None, linewidth=100, num_objects=None, palette=None):
    ...
    num_objects += 1  # background
    if palette is None:
        palette = [(0.0, 0.0, 0.0)] + sns.color_palette("hls", num_objects)
    seg = seg[:, :, 0]
    for i in range(1, num_objects):
        ...
        if idxs.size > 0:
            ...
            rect = matplotlib.patches.Rectangle(
                [minx, miny], maxx - minx, maxy - miny, linewidth=linewidth, edgecolor=palette[i], facecolor="none"
            )
            ax.add_patch(rect)
...
def plot_object_collisions(collisions, ax, frame, num_objects=None, palette=None):
    ...
    if palette is None:
        palette = [(0.0, 0.0, 0.0)] + sns.color_palette("hls", num_objects)
    coll = [c for c in collisions if (-1 not in c["instances"]) and (frame - 0.5 <= c["frame"] <= frame + 0.5)]
    for c in coll:
        ax.scatter(
            x=c["image_position"][0],
            y=c["image_position"][1],
            s=c["force"] * 1000,
            marker="*",
            facecolors=palette[c["instances"][0] + 1],
            edgecolors=palette[c["instances"][1] + 1],
            linewidth=50,
        )


def plot_ground_collisions(collisions, ax, frame):
    coll = [c for c in collisions if (-1 in c["instances"]) and (frame - 0.5 <= c["frame"] <= frame + 0.5)]
    for c in coll:
        ax.scatter(x=c["image_position"][0], y=c["image_position"][1], s=c["force"] * 1000, marker=".", facecolors="k")
