...
class ArgumentParser(argparse_flags.ArgumentParser):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.add_argument(
            "--frame_rate", type=int, default=24, help="number of rendered frames per second (default: 24)"
        )
        self.add_argument(
            "--step_rate",
            type=int,
        ...
        self.add_argument(
            ...
            type=int,
            default=1,
            help="index of the first frame to render. " "Note that simulation always starts at frame 0 (default: 1)",
        )
        self.add_argument("--frame_end", type=int, default=24, help="index of the last frame to render (default: 24)")
        self.add_argument("--logging_level", type=str, default="INFO")
        self.add_argument(
            "--seed", type=int, default=None, help="(int) seed for random sampling in the worker (default: None)"
        )
        self.add_argument(
            "--resolution",
            type=str,
...
def done():
    ...
    hpt.report_hyperparameter_tuning_metric(hyperparameter_metric_tag="answer", metric_value=42)
...
def get_instance_info(scene, assets_subset=None):
    ...
    for instance in assets_subset:
        ...
        info["image_positions"] = np.array(
            [scene.camera.project_point(point3d=p, frame=f)[:2] for f, p in zip(frame_range, info["positions"])],
            dtype=np.float32,
        )
        bboxes3d = []
