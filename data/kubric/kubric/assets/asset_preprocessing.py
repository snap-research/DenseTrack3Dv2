...
def get_vertices_and_faces(obj):
    ...
    faces = np.array(
        [list(p.vertices) for p in bmesh.polygons if len(p.vertices) > 2]
...
def save_collision_geometry(obj, output_path):
    ...
    with select(obj), center(obj):
        bpy.ops.export_scene.obj(
            filepath=str(collision_path), axis_forward="Y", axis_up="Z", use_selection=True, use_materials=False
        )
...
def save_visual_geometry(obj, output_path):
    ...
    with select(obj), center(obj):
        bpy.ops.export_scene.obj(
            filepath=str(vis_path), axis_forward="Y", axis_up="Z", use_selection=True, use_materials=True
        )
    return vis_path
