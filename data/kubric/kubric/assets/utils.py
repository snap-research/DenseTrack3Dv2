...
import kubric as kb
import numpy as np
from kubric import core, randomness
...
def get_lfn_lights(light_jitter: float = 1.0, rng: np.random.RandomState = randomness.default_rng()):
    ...
    lamp_back = core.RectAreaLight(
        name="lamp_back", color=color.Color.from_name("white"), intensity=100.0, position=(-1.1685, 2.64602, 5.81574)
    )
    lamp_key = core.RectAreaLight(
        name="lamp_key",
...
def get_clevr_lights(light_jitter: float = 1.0, rng: np.random.RandomState = randomness.default_rng()):
    """Create lights that match the setup from the CLEVR dataset."""
    sun = core.DirectionalLight(
        name="sun",
        color=color.Color.from_name("white"),
    ...
    lamp_back = core.RectAreaLight(
        name="lamp_back", color=color.Color.from_name("white"), intensity=50.0, position=(-1.1685, 2.64602, 5.81574)
    )
    lamp_key = core.RectAreaLight(
        name="lamp_key",
        color=color.Color.from_hexint(0xFFEDD0),
...
def get_random_kubasic_object(
    ...
    obj = asset_source.create(
        name=f"{size_label} {color_label} {material_name} {shape_name}", asset_id=shape_name, scale=size
    )
