...
class AssetSource(ClosableResource):
    ...
    @classmethod
    def from_manifest(cls, manifest_path: PathLike, scratch_dir: Optional[PathLike] = None) -> "AssetSource":
    ...
    def __init__(self, name: str, data_dir: PathLike, assets: Dict[str, Any], scratch_dir: Optional[PathLike] = None):
        super().__init__()
        self.name = name
        self.data_dir = file_io.as_path(data_dir)
        logging.info("Created AssetSource '%s' with '%d' assets at URI='%s'", name, len(assets), self.data_dir)
    ...
    def db(self):
        ...
        db = pd.DataFrame([{"id": k} | v["kwargs"] | v["metadata"] for k, v in self._assets.items()])
    ...
    @functools.cached_property
    def categories(self):
        return sorted(filter(None, {v["metadata"].get("category", "") for v in self._assets.values()}))
    ...
    def _resolve_asset_type(asset_type: str) -> Type:
        ...
        if asset_type not in types:
            raise KeyError(f"Unknown asset_type {asset_type!r}. " f"Available types: {types!r}")
