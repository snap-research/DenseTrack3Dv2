...
class PyBullet(core.View):
    ...
    def _add_object(self, obj: core.Cube) -> Optional[int]:
        collision_idx = self._physics_client.createCollisionShape(pb.GEOM_BOX, halfExtents=obj.scale)
        visual_idx = -1
        ...
        box_idx = self._physics_client.createMultiBody(
            mass, collision_idx, visual_idx, obj.position, wxyz2xyzw(obj.quaternion), useMaximalCoordinates=True
        )
        self._physics_client.changeDynamics(box_idx, -1, contactProcessingThreshold=0)
    ...
    def _add_object(self, obj: core.Sphere) -> Optional[int]:
        ...
        collision_idx = self._physics_client.createCollisionShape(pb.GEOM_SPHERE, radius=radius)
        visual_idx = -1
        mass = 0 if obj.static else obj.mass
        # useMaximalCoordinates and contactProcessingThreshold are required to
        # fix the sticky walls issue;
        ...
        self._physics_client.changeDynamics(sphere_idx, -1, contactProcessingThreshold=0)
    ...
    def _add_object(self, obj: core.FileBasedObject) -> Optional[int]:
        ...
        if path.suffix == ".urdf":
            obj_idx = self._physics_client.loadURDF(
                str(path), useFixedBase=obj.static, globalScaling=scale, useMaximalCoordinates=True
            )
        else:
            raise IOError("Unsupported format '{path.suffix}' of file '{path}'")

        if obj_idx < 0:
            raise IOError(f"Failed to load '{path}'")

        self._physics_client.changeDynamics(obj_idx, -1, contactProcessingThreshold=0)
    ...
    def check_overlap(self, obj: core.PhysicalObject) -> bool:
        ...
        body_ids = [self._physics_client.getBodyUniqueId(i) for i in range(self._physics_client.getNumBodies())]
        for body_id in body_ids:
            if body_id == obj_idx:
                continue
            overlap_points = self._physics_client.getClosestPoints(obj_idx, body_id, distance=0)
            if overlap_points:
                return True
        return False
    ...
    def run(
        self, frame_start: int = 0, frame_end: Optional[int] = None
        ...
        obj_idxs = [self._physics_client.getBodyUniqueId(i) for i in range(self._physics_client.getNumBodies())]
        animation = {
            obj_id: {"position": [], "quaternion": [], "velocity": [], "angular_velocity": []} for obj_id in obj_idxs
        }
        ...
        animation = {
            asset: animation[asset.linked_objects[self]]
...
def register_physical_object_setters(obj: core.PhysicalObject, obj_idx, physics_client: _BulletClient):
...
def set_angular_velocity(
    object_idx, angular_velocity, asset, physics_client: _BulletClient
...
def set_static(object_idx, is_static, asset, physics_client: _BulletClient):
    if is_static:
        physics_client.changeDynamics(object_idx, -1, mass=0.0)
    else:
...
def set_restitution(
    object_idx, restitution: float, asset, physics_client: _BulletClient
):  # pylint: disable=unused-argument
    if restitution < 0:
