...
def write_png(data: np.array, filename: PathLike) -> None:
    ...
    if data.dtype in [np.uint32, np.uint64]:
        ...
        if max_value > 65535:
            logger.warning("max_value %d exceeds uint16 bounds for %s.", max_value, filename)
        ...
        if min_value < 0.0 or max_value > 1.0:
            raise ValueError(
                f"Values need to be in range [0, 1] but got [{min_value}, {max_value}] " f"for {filename}"
            )
...
def write_palette_png(data: np.array, filename: PathLike, palette: np.ndarray = None):
    ...
    if data.dtype in [np.uint16, np.uint32, np.uint64]:
        max_value = np.amax(data)
        if max_value > 255:
            logger.warning("max_value %d exceeds uint bounds for %s.", max_value, filename)
...
def multi_write_image(data: np.ndarray, path_template: str, write_fn=write_png, max_write_threads=16, **kwargs):
...
def write_coordinates_batch(data, directory, file_template="object_coordinates_{:05d}.png", max_write_threads=16):
    assert data.ndim == 4 and data.shape[-1] == 3, data.shape
    path_template = str(as_path(directory) / file_template)
    multi_write_image(data, path_template, write_fn=write_png, max_write_threads=max_write_threads)
...
def write_segmentation_batch(data, directory, file_template="segmentation_{:05d}.png", max_write_threads=16):
    assert data.ndim == 4 and data.shape[-1] == 1, data.shape
    assert data.dtype in [np.uint8, np.uint16, np.uint32, np.uint64], data.dtype
    path_template = str(as_path(directory) / file_template)
    palette = plotting.hls_palette(np.max(data) + 1)
    multi_write_image(
        data, path_template, write_fn=write_palette_png, max_write_threads=max_write_threads, palette=palette
    )


def write_flow_batch(
    data, directory, file_template="flow_{:05d}.png", name="flow", max_write_threads=16, range_file="data_ranges.json"
):
...
write_forward_flow_batch = functools.partial(
    write_flow_batch, name="forward_flow", file_template="forward_flow_{:05d}.png"
)
write_backward_flow_batch = functools.partial(
    write_flow_batch, name="backward_flow", file_template="backward_flow_{:05d}.png"
)
...
def write_image_dict(
    data_dict: Dict[str, np.ndarray], directory: PathLike, file_templates: Dict[str, str] = (), max_write_threads=16
):
    for key, data in data_dict.items():
        if key in file_templates:
            DEFAULT_WRITERS[key](
                data, directory, file_template=file_templates[key], max_write_threads=max_write_threads
            )
        else:
