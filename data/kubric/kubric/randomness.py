...
from kubric.core import color, objects
...
def random_hue_color(saturation: float = 1.0, value: float = 1, rng=default_rng()):
...
def random_rotation(axis=None, rng=default_rng()):
    ...
    if axis is None:
        ...
        if isinstance(axis, str) and axis.upper() in ["X", "Y", "Z"]:
            axis = {"X": (1.0, 0.0, 0.0), "Y": (0.0, 1.0, 0.0), "Z": (0.0, 0.0, 1.0)}[axis.upper()]
...
def move_until_no_overlap(asset, simulator, spawn_region=((-1, -1, -1), (1, 1, 1)), max_trials=100, rng=default_rng()):
...
def sample_color(strategy: str, rng: np.random.RandomState = default_rng()) -> Tuple[Optional[str], color.Color]:
...
def sample_sizes(strategy: str, rng: np.random.RandomState = default_rng()) -> Tuple[Optional[str], float]:
...
def sample_point_in_half_sphere_shell(
    inner_radius: float, outer_radius: float, offset: float = 0.0, rng: np.random.RandomState = default_rng()
    ...
    while True:
        ...
        radius = rng.uniform(inner_radius**3, outer_radius**3) ** (1 / 3.0)
