...
def load_scene_directory(scene_dir, target_size, layers=DEFAULT_LAYERS):
    ...
    if "depth" in layers:
        ...
        depth_frames = np.array(
            [subsample_nearest_neighbor(file_io.read_tiff(frame_path), target_size) for frame_path in depth_paths]
        )
    ...
    if "forward_flow" in layers:
        ...
        result["forward_flow"] = [
            subsample_nearest_neighbor(file_io.read_png(frame_path)[..., :2], target_size)
    ...
    for key in ["normal", "object_coordinates", "uv"]:
        if key in layers:
            result[key] = [
                subsample_nearest_neighbor(file_io.read_png(frame_path), target_size) for frame_path in paths[key]
            ]
    ...
    if "segmentation" in layers:
        ...
        result["segmentations"] = [
            subsample_nearest_neighbor(file_io.read_png(frame_path), target_size)
            for frame_path in paths["segmentation"]
        ]

    if "rgba" in layers:
        result["video"] = [
            subsample_avg(file_io.read_png(frame_path), target_size)[..., :3] for frame_path in paths["rgba"]
        ]
...
def get_camera_features(seq_length):
    return {
        ...
        "positions": tfds.features.Tensor(shape=(seq_length, 3), dtype=tf.float32),
        "quaternions": tfds.features.Tensor(shape=(seq_length, 4), dtype=tf.float32),
    }
...
def get_instance_features(seq_length: int):
    return {
        ...
        "positions": tfds.features.Tensor(shape=(seq_length, 3), dtype=tf.float32),
        "quaternions": tfds.features.Tensor(shape=(seq_length, 4), dtype=tf.float32),
        "velocities": tfds.features.Tensor(shape=(seq_length, 3), dtype=tf.float32),
        "angular_velocities": tfds.features.Tensor(shape=(seq_length, 3), dtype=tf.float32),
        "bboxes_3d": tfds.features.Tensor(shape=(seq_length, 8, 3), dtype=tf.float32),
        "image_positions": tfds.features.Tensor(shape=(seq_length, 2), dtype=tf.float32),
        "bboxes": tfds.features.Sequence(tfds.features.BBoxFeature()),
        "bbox_frames": tfds.features.Sequence(tfds.features.Tensor(shape=(), dtype=tf.int32)),
...
def subsample_avg(arr, size):
    ...
    return np.round(arr.reshape((dst_height, height_bin, dst_width, width_bin, channels)).mean(axis=(1, 3))).astype(
        np.uint8
    )
