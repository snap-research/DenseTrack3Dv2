...
class KubricBuilder(tfds.core.GeneratorBasedBuilder):
    ...
    * `generate_scene`: Generate a single scene and returns the examples.
    ...
    def _split_generators(
        ...
        data_per_split = (
            pipeline
            | "Creating scenes" >> beam.Create(split_to_scenes.scene_id_to_scene_config.items())
            ...
            | "Partition by split"
            >> beam.Partition(
                lambda ex, _: split_to_scenes.split_name_to_split_index[
                    ex[0]
        ...
        split_to_exs = {
            split_name: examples | f"Remove {split_name}" >> beam.Map(lambda ex: ex[1:])
    ...
    def _normalize_scene_output(
        ...
        *,
        ...
        if isinstance(scene_output, dict) and set(scene_output) == set(expected_splits):
            ...
            # during encoding.
            return {split_name: self._normalize_scene_examples(exs) for split_name, exs in scene_output.items()}
        else:
