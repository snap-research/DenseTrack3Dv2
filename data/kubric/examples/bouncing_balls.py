...
# --- construct the room
floor_material = kb.FlatMaterial(color=kb.get_color("black"), indirect_visibility=False)
wall_material = kb.FlatMaterial(color=kb.get_color("white"), indirect_visibility=False)
room_dynamics = {
...
north_wall = kb.Cube(
    scale=(1.2, 0.9, 1), position=(0, 1.9, 0.9), material=wall_material, static=True, background=True, **room_dynamics
)
south_wall = kb.Cube(
    scale=(1.2, 0.9, 1), position=(0, -1.9, 0.9), material=wall_material, static=True, background=True, **room_dynamics
)
east_wall = kb.Cube(
    scale=(0.9, 1, 1), position=(1.9, 0, 0.9), material=wall_material, static=True, background=True, **room_dynamics
)
west_wall = kb.Cube(
    scale=(0.9, 1, 1), position=(-1.9, 0, 0.9), material=wall_material, static=True, background=True, **room_dynamics
)
...
if FLAGS.color == "uniform_hsv":
    ...
    hues = np.linspace(0, 1.0, nr_objects, endpoint=False)
    colors = [kb.Color.from_hsv(hue, 1.0, 1.0) for hue in hues]
elif FLAGS.color.startswith("cat"):
    num_colors = int(FLAGS.color[3:])
    all_hues = np.linspace(0, 1.0, num_colors, endpoint=False)
...
def get_random_ball(color):
    ...
    ball_material = kb.FlatMaterial(color=color, indirect_visibility=False)
    shape = FLAGS.shape
    if shape == "mixed":
        shape = rng.choice(["cube", "sphere"])
    if shape == "cube":
        ball = kb.Cube(
            scale=[FLAGS.ball_radius] * 3,
...
# --- simulation
logging.info("Saving the simulator state to '%s' before starting the simulation.", output_dir / "scene.bullet")
...
# --- Rendering
logging.info("Saving the renderer state to '%s' before starting the rendering.", output_dir / "scene.blend")
