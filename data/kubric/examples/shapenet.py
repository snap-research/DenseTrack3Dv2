...
airplane_ids = [name for name, spec in shapenet._assets.items() if spec["metadata"]["category"] == "airplane"]
...
data_stack["segmentation"] = kb.adjust_segmentation_idxs(data_stack["segmentation"], scene.assets, [obj]).astype(
    np.uint8
)
