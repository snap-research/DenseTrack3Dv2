...
from densetrack3d.models.loss import balanced_bce_loss, seq_balanced_bce_loss, bce_loss, seq_bce_loss, confidence_loss, seq_confidence_loss, track_loss
...
KUBRIC3D_MIX_DIR = "4d-data/dense_tracking_datasets/kubric_processed_mix/"
...
def forward_batch(batch, model, args):
    ...
    for idx, ind in enumerate(range(0, args.sequence_len - S // 2, S // 2)):
        ...
        coord_predictions_ = coord_predictions[idx][:, :, :, :n_sparse_queries, :] # B I T N C
        ...
        seq_loss += track_loss(coord_predictions_, traj_gt_, valid_gt_, divide_n_repeat=True, use_huber_loss=False, delta_huber_loss=6.0/W)
        # seq_depth_loss += track_loss(1 / coord_depth_predictions_, 1 / traj_d_gt_, valid_gt_)
        vis_loss += seq_balanced_bce_loss(vis_predictions_, vis_gt_, valid_gt_, divide_n_repeat=True)
        conf_loss += seq_confidence_loss(
            coord_predictions_, conf_predictions_, traj_gt_, vis_gt_, valid_gt_, expected_dist_thresh=12.0 / (W - 1), divide_n_repeat=True
        )
    ...
    for idx, ind in enumerate(range(0, args.sequence_len - S // 2, S // 2)):
        ...
        # )
        dense_vis_loss_ = seq_bce_loss(dense_vis_prediction_, gt_alpha, divide_n_repeat=True, has_batch_dim=False)  # bce_loss(dense_vis_prediction_, gt_alpha)
    ...
    if "coords_down" in dense_train_data_dict.keys():

        dense_down_coord_predictions = dense_train_data_dict["coords_down"]
        # dense_coord_depth_predictions = dense_train_data_dict["coord_depths"]
        dense_down_vis_predictions = dense_train_data_dict["vis_down"]
        dense_down_conf_predictions = dense_train_data_dict["conf_down"]

        dense_down_seq_loss = torch.tensor(0.0, requires_grad=True).cuda()
        # dense_seq_depth_loss = torch.tensor(0.0, requires_grad=True).cuda()
        dense_down_vis_loss = torch.tensor(0.0, requires_grad=True).cuda()
        dense_down_conf_loss = torch.tensor(0.0, requires_grad=True).cuda()

        for idx, ind in enumerate(range(0, args.sequence_len - S // 2, S // 2)):

            if idx >= len(dense_down_coord_predictions):
                break

            dense_down_coord_prediction_ = dense_down_coord_predictions[idx][0]  # I T, 3, H, W
            # dense_coord_depth_prediction_ = dense_coord_depth_predictions[idx][0]  # I T, 3, H, W
            dense_down_vis_prediction_ = dense_down_vis_predictions[idx][0]  # T,  H, W
            dense_down_conf_prediction_ = dense_down_conf_predictions[idx][0]  # T,  H, W

            pred_down_H, pred_down_W = dense_down_coord_prediction_.shape[-2:]

            # gt_dense_traj_d = dense_trajectory_d[
            #     0, ind : ind + S, :, y0[0] * model_stride : (y0[0] * model_stride + pred_H), x0[0] * model_stride : (x0[0] * model_stride + pred_W)
            # ].clone()  # T 1 H_crop W_crop
            gt_alpha = flow_alpha[
                0, ind : ind + S, y0[0] * model_stride : (y0[0] * model_stride + pred_down_H * model_stride), x0[0] * model_stride : (x0[0] * model_stride + pred_down_W * model_stride)
            ].clone()  # T 2 H W
            gt_flow = flow[
                0, ind : ind + S, :, y0[0] * model_stride : (y0[0] * model_stride + pred_down_H * model_stride), x0[0] * model_stride : (x0[0] * model_stride + pred_down_W * model_stride)
            ].clone()  # T 2 H_crop W_crop

            gt_down_alpha = F.interpolate(
                gt_alpha.unsqueeze(1), 
                scale_factor=1/model_stride, 
                mode="nearest"
            )
            gt_down_alpha = gt_down_alpha.squeeze(1)

            gt_down_flow = F.interpolate(
                gt_flow, 
                scale_factor=1/model_stride, 
                mode="bilinear"
            )
            gt_down_flow = gt_down_flow / model_stride


            # I, S = dense_down_coord_prediction_.shape[:2]

            dense_down_grid_2d = get_grid(pred_down_H, pred_down_W, normalize=False, device=dense_down_coord_prediction_.device)  # H W 2
            dense_down_grid_2d[..., 0] += x0[0]
            dense_down_grid_2d[..., 1] += y0[0]
            dense_down_grid_2d = rearrange(dense_down_grid_2d, "h w c -> () c h w")

            gt_down_coord_ = gt_down_flow + dense_down_grid_2d

            gt_down_coord_[:, 0] /= (W/model_stride) - 1
            gt_down_coord_[:, 1] /= (H/model_stride) - 1


            dense_down_coord_prediction_[:, :, 0] /= (W/model_stride) - 1
            dense_down_coord_prediction_[:, :, 1] /= (H/model_stride) - 1
            ...
            dense_down_seq_loss_ = track_loss(
                dense_down_coord_prediction_, 
                gt_down_coord_, 
                is_dense=True, 
                has_batch_dim=False, 
                divide_n_repeat=True, 
            ...
            dense_down_seq_loss += dense_down_seq_loss_
            # dense_seq_depth_loss += track_loss(
            #     1 / dense_coord_depth_prediction_, 1 / gt_dense_traj_d, is_dense=True, has_batch_dim=False
            # )
            dense_down_vis_loss_ = seq_bce_loss(dense_down_vis_prediction_, gt_down_alpha, divide_n_repeat=True, has_batch_dim=False)  # bce_loss(dense_vis_prediction_, gt_alpha)
            dense_down_vis_loss += dense_down_vis_loss_

            dense_down_conf_loss += seq_confidence_loss(
                dense_down_coord_prediction_,
                dense_down_conf_prediction_,
                gt_down_coord_,
                gt_down_alpha,
                expected_dist_thresh=12.0 / W,
                is_dense=True,
        ...
        dense_down_seq_loss = dense_down_seq_loss * 0.5 * args.lambda_2d / len(dense_down_coord_predictions)  # FIXME
        # dense_seq_depth_loss = dense_seq_depth_loss * args.lambda_d / len(dense_coord_predictions)
        dense_down_vis_loss = dense_down_vis_loss * 0.5 * args.lambda_vis / len(dense_down_coord_predictions)
        dense_down_conf_loss = dense_down_conf_loss * 0.5 * args.lambda_conf / len(dense_down_coord_predictions)

    output.update(
        {
            "dense_down_track_uv": {
                "loss": dense_down_seq_loss.mean(),
            },
            "dense_down_vis": {
                "loss": dense_down_vis_loss.mean(),
            },
            "dense_down_conf": {
                "loss": dense_down_conf_loss.mean(),
            },
        }
    )
